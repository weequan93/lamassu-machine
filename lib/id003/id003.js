...
Id003.prototype._run = function _run (cb) {
  ...
  this.rs232.on('message', function (cmd, data) {
    ...
    if (cmd === 'invalid') {
      ...
      if (self.disablePolling) self._send('status')
      return
    }
    ...
    if (cmd === 'commMode') {
      self._disablePolling()
    }
    if (cmd === 'enq') return self._send('status')
  })
  ...
  this.id003Fsm.on('initialize', function () {
...
Id003.prototype._send = function _send (command, data) {
  if (this.disablePolling) return this.rs232.send(command, data)
...
Id003.prototype._disablePolling = function () {
