...
const pairing = require('./hyper/hyperPairing')
const Tx = require('./hyper/hyperTx')
const BN = require('./bn')
...
const Brain = function (config) {
  ...
  this.dataPath = path.resolve(process.cwd(), this.config.dataPath)
  this.certPath = {
  ...
  this.scanner = config.mockCam
    ? require('./hyper/mocks/scanner')
    : require('./hyper/hyperScanner')
...
Brain.prototype.checkDownloadSpeed = function checkDownloadSpeed (site) {
  return ping.checkDownloadSpeed(site)
    .then(res => {
      if (!res) return
...
Brain.prototype.networkHeartbeat = function networkHeartbeat (urls) {
  return ping.pingRepository(urls)
    .then(res => {
      if (!res) return
...
Brain.prototype._initBillValidatorEvents = function _initBillValidatorEvents () {
  ...
  billValidator.on('initialize', function () { self._initialize() })
...
Brain.prototype._idle = function _idle (locale) {
  ...
  this.tx = Tx.newTx(this.dbRoot + "/tx-hyperdb")
  this.pk = null
  this.bill = null
...
Brain.prototype.sendRates = function sendRates () {
  console.log("should not sendrate from here")
  const cryptoCode = this.tx.cryptoCode
...
Brain.prototype._transitionState = function _transitionState (state, auxData) {
  console.log("_transitionState from ",this.state, " > ", state)
  // TODO refactor code to use this
  // If we're in maintenance state, we stay there till we die
  if ((this.state === state || this.state === 'maintenance') && auxData &&  auxData.force != true) return false
...
Brain.prototype._initialize = function _initialize(){
...
Brain.prototype.postTx = function postTx (tx) {
  ...
  console.debug("postTx")
  console.debug("postTx tx", tx.id)
  ...
  return source$
    ...
    .toPromise()
    .then(r => {
      console.debug("postTx tx", r.tx.id)
      return {}
      if (r.tx) return r.tx
...
Brain.prototype.updateTx = function updateTx (updateTx) {
  ...
  // console.debug("updateTx tx", updateTx.id)
  ...
  return this.saveTx(newTx)
    ...
    .then(tx => {
      this.tx = tx
      // console.debug("updateTx tx", this.tx.id)
      return tx
    })
}
...
Brain.prototype._printBuyReceipt = function _printBuyReceipt(data) {
  if (!this.printer) {
  ...
  const printData = {
    ...
    atmNo: this.atmTrader.globalOptions.connectionInfo["ATM-Number"],
    fiat: this.tx.fiat,
    fiatCurrency: this.atmTrader.config.fiatCurrency,
    order_no: this.atmTrader.tx.orderNo,
    created: dateString,
    chain: chain.chain,
    coin: this.atmTrader.tx.coinname,
    Icon: dataPath + "/" + "assets/image/header.png",
    Icon2: dataPath + "/" + "assets/image/app.png",
    QRCode: this.atmTrader.globalOptions.connectionInfo["Mobile-App-Site"]
  }

  const templatePath = `${dataPath}/template/order_buy.${data.language}.tpl` 
  const templateRaw = fs.readFileSync(templatePath, "utf-8")
  const templatePlaceholder = Handlebars.compile(templateRaw);
  const template = templatePlaceholder(printData)

  return this.printer.checkStatus(deviceConfig.kioskPrinter, STATUS_QUERY_TIMEOUT)
    .then(() => this.printer.print(template, printData, deviceConfig.kioskPrinter))
    .catch((err) => {
      console.log('[ERROR]: The kiosk printer is in an invalid state.', err)
      throw err
    })
...
Brain.prototype._printReceipt = function _printReceipt (data) {
  if (!this.printer) {
    console.log('[ERROR]: The kiosk printer is not loaded')
    return
  }
  ...
  const printData = {
    Icon: dataPath + "/" + "assets/image/header.png",
    created: dateString,
    order_no: this.atmTrader.tx.orderDetail.order_no,
    fiat: this.atmTrader.tx.orderDetail.fiat_amount,
    fiatCurrency: this.atmTrader.tx.orderDetail.fiat_currency,
    atmNo: this.atmTrader.globalOptions.connectionInfo["ATM-Number"],
    QRCode: this.atmTrader.tx.orderNo,
    chain: chain.chain,
    coin: this.atmTrader.tx.orderDetail.digital_currency,
    QRCode2: this.atmTrader.tx.orderDetail.address,
    Icon2: dataPath + "/" + "assets/image/app.png",
    QRCode3: this.atmTrader.globalOptions.connectionInfo["Mobile-App-Site"]
  }

  const templatePath = `${dataPath}/template/order_sell.${data.language}.tpl`
  const templateRaw = fs.readFileSync(templatePath, "utf-8")
  const templatePlaceholder = Handlebars.compile(templateRaw);
  const template = templatePlaceholder(printData)
...
Brain.prototype._reboot = async function _reboot () {
  if(this.tx.start == true){
    this.heartBeat(1)
    // wait for end
...
Brain.prototype._shutdown = async function _shutdown () {
  if (this.tx.start == true) {
    // wait for end
    this.heartBeat(3)
    await new Promise((resolve) => {
...
Brain.prototype._toMaintenance = async function _toMaintenance(){
  if (this.tx.start == true) {
    this.heartBeat(7)
    // wait for end
    await new Promise((resolve) => {
  ...
  this.heartBeat(7)
  console.log('Going maintenance [%s]...', reason)

  const command = this.determinePlatform === 'AAEON'
    ? 'killall chromium-browser' : 'supervisorctl stop hyper-browser'

  cp.execFile(command, [], {}, function (msg) {
    console.log(command, msg)
    const command2 = 'supervisorctl start hyper-maintenance'
    cp.execFile(command2, [], {}, function (msg) {
      console.log(command2, msg)
      return process.exit(0)
    })
  })
}

Brain.prototype._doneMaintenance = async function _doneMaintenance() {
  if (this.tx.start == true) {
    this.heartBeat(0)
    // wait for end
    await new Promise((resolve) => {
  ...
  const command = this.determinePlatform === 'AAEON'
    ? 'killall chromium-browser' : 'supervisorctl restart hyper-browser'

  cp.execFile(command, [], {}, function (msg) {
    console.log(command, msg)
    const command2 = 'supervisorctl stop hyper-maintenance'
    cp.execFile(command2, [], {}, function (msg) {
      console.log(command2, msg)
      return process.exit(0)
    })
...
Brain.prototype._restartServices = async function _restartServices (reason, restartBrowser) {
  ...
  const command = this.determinePlatform === 'AAEON'
    ? 'killall chromium-browser' : 'supervisorctl restart hyper-browser hyper-machine hyper-janitor hyper-watchdog hyper-updater'
...
Brain.prototype._getFiatButtonResponse = function _getFiatButtonResponse () {
  ...
  console.log("=== _getFiatButtonResponse", activeDenominations)
...
Brain.prototype.exceedsZeroConf = function exceedsZeroConf (tx) {
  ...
  return coin.zeroConf && tx.fiat.gte(this.trader.zeroConfLimit || 1)
}
...
Brain.prototype._waitForDispense = function _waitForDispense (status) {
  console.debug("_waitForDispense")
...
Brain.prototype._dispense = function _dispense () {
  ...
  return Promise.resolve()
    .then(() => {
      ...
      console.log("=== === _dispense before update transaction")
...
Brain.prototype._batchDispense = function _batchDispense (notesToDispense, { batchAmount, dispenseRecords, currentBatch, error }) {
  ...
  return this.billDispenser.dispense(notesToDispense, currentBatch, batchAmount)
    .then(({ value, error }) => {
      console.log("=== _batchDispense", value, error)
      const response = {
        batchAmount,
        batch: value,
...
Brain.prototype._batchesFinished = function _batchesFinished ({ dispenseRecords, error }) {
  console.debug("_batchesFinished", dispenseRecords)
  const self = this;
  const tx = this.tx
  const bills = _.toArray(_.merge(tx.bills, dispenseRecords))
  console.debug("bills", tx.bills)
  ...
  pDelay(this.config.completedTimeout).then(() => {
    ...
    this.toDeposit()
    this.saveHyperOrderTx(self.atmTrader.tx.orderDetail, dispenserInfo, new Date().toISOString())
...
Brain.prototype._physicalDispense = function _physicalDispense () {
  console.log("=== _physicalDispense ")

  const fiatCode = this.tx.fiatCode
  console.log("=== _physicalDispense ", this.tx.bills, this.numberOfCassettes)
  const notes = []
  _.forEach(it => notes.push(this.tx.bills[it].provisioned), _.times(_.identity(), this.numberOfCassettes))
  console.log("this.billDispenser", this.billDispenser)
  ...
  console.log("=== notesToDispense",notesToDispense)
  const batchAmount = notesToDispense.length
  console.log("=== batchAmount",batchAmount)
  const initialParams = {
    notesToDispense,
    batchAmount,
    currentBatch: 1,
  ...
  // can capture the _batchDispense here
  const batches = notesToDispense.reduce((acc, notes) => acc.then(it => {
    const r = this._batchDispense(notes, it)
    console.debug(">>>>> r", r)
    return r
