...
const saveHyperTxHyperBrain = function (config) {
    const self = this
    ...
    this.scanner.atmStartScanQR((err, address) => {
        ...
        if (address) {
            ...
            if (selfCashOutMachine.isInState() == true) {
                ...
                //         self.atmError(hyperError.errMsg["atmScanInvalidAddress"], err)
                //         self.atmStopScanQR()
                //         self.atmStartScanQR()
                //     }else{
...
HyperBrain.prototype.heartBeat = async function (status) {
    if (status) {
        this.blockState = status
    } else {
        status = this.blockState
    }

    console.debug("heartBeat send status=", status)
    // device_status 0:正常， 1:安排重启，2:重启， 3:安排关机，4:关机， 5:安排服务重启， 6:服务重启，7:维护中,
    // bill validator 8: inbox open ,9: inbox close 10: jam, 11, disconnect
    // 
    ...
    const dataSend = {
        latency: 0,// parseFloat(loadsh.get(networkHeartBeat, "0.averageResponseTime", 0)),
        network_speed:0,// parseFloat(lodash.get(downloadSpeed, "0.speed"), 0),
        packet_loss: 0,//parseFloat(lodash.get(networkHeartBeat, "0.packetLoss"), 0),
        device_status: status,
    }

    this.socket.send(JSON.stringify({ command: "device_status", body: dataSend }));
    // update normal when terminate state happen
...
HyperBrain.prototype.atmChooseBuyCoin = function (data) {
    ...
    this.atmTrader.getSnapshotInfo({
        ...
        this.atmTrader.updateTx({ snapshotInfoId: res.body.data.id })

        self.atmTrader.config.fiatUSDRate = res.body.data.coin_out.fiat_usdt_rate || 1

        // update rate
        self.atmTrader.config.coinIndex[this.atmTrader.tx.coinname].price = res.body.data.coin_out.rate
        self.atmTrader.config.coinIndex[this.atmTrader.tx.coinname].sellFee = res.body.data.coin_out.sell_rate || 0
        self.atmTrader.config.coinIndex[this.atmTrader.tx.coinname].buyFee = res.body.data.coin_out.buy_rate || 0
        self.atmTrader.config.coinIndex[this.atmTrader.tx.coinname].sellFeeRate = res.body.data.coin_out.sell_fee_rate || 0
        self.atmTrader.config.coinIndex[this.atmTrader.tx.coinname].buyFeeRate = res.body.data.coin_out.buy_fee_rate || 0

        self.atmTrader.config.coins = Object.values(self.atmTrader.config.coinIndex)

        this.emit("atmSync")
        fs.writeFileSync("/tmp/extract/running.txt", 'RUNNING\n')
...
HyperBrain.prototype.atmConfirmAddress = function (data) {
    // pre create order
    this.atmTrader.initAtmOrderBuy({"address": data.toAddress}).then((res)=>{
        console.debug(">>>>> initAtmOrderBuy", res.body)
        // res.body.data.order_no
        const orderNo = lodash.get(res, "body.data.order_no", null)
        if (orderNo == null){
            console.error("initAtmOrderBuy is empty orderNo=",orderNo)
            this.atmError(hyperError.errMsg["atmCreateOrderFailNoOrderNo"], {})
            return
        }
        // update chain
        // update toAddress
        this.tx.toAddress = data.toAddress
        this.atmTrader.updateTx({ chain: data.chain, type: "atm_order_buy", orderNo: orderNo })
        this.saveHyperTx({})
        cashInMachine.dispatch("CONFIRM")
    }).catch((e)=>{
        console.error("initAtmOrderBuy error", e)
        this.atmError(hyperError.errMsg["atmCreateOrderFail"], {})
        return
    })
}
...
HyperBrain.prototype.updateBillLogic = function () {
    this.saveHyperTx({})
    //this._transitionState('acceptingBills', { path: "/depositSuccessful", tx: this.tx, orderNo: this.atmTrader.tx.orderNo, })
    this._transitionState("acceptingBills", {
        data: { 
            path: "/depositSuccessful", tx: this.tx, orderNo: this.atmTrader.tx.orderNo, 
            render: this.buildRender(false, {autoNext: false, timeout: 300})
        }
    })
}
...
HyperBrain.prototype.atmWithdrawaActionLogic = function () {
    ...
    if (this.atmTrader.tx.orderDetail.order_no == "20220901062535153786") {
        this.tx.fiat = BN(2);
        this.atmTrader.tx.fiat_amount = BN(2);
    }
    // get cassetle count
    ...
    this.atmTrader.beforeSellSplitMoney({
        ...
        "order_no": self.atmTrader.tx.orderDetail.order_no
    }).then((respond) => {
...
HyperBrain.prototype.atmCleanUpLogic = function () {
    ...
    this.atmTrader.stopPoolLogin()
    this.atmTrader.resetTx()
    fs.unlinkSync("/tmp/extract/running.txt")
...
HyperBrain.prototype.atmConfirmDepositLogic = function () {
    ...
    // only for real test
    const decimalN = self.prepareConvertDecimalDigit(coin.decimals)
    ...
    const dAmount = (self.tx.fiat.toNumber() / self.atmTrader.tx.coinprice * (1 - (coin.buyFee + coin.buyFeeRate)));
    const rData = {
        order_no: self.atmTrader.tx.orderNo,
        ...
        digital_amount: decimalN.convert(dAmount),
        ...
        price: decimalN.convert(self.tx.fiat.toNumber()/dAmount),
        price_origin: self.atmTrader.tx.coinprice,
        fiat_usdt_rate: self.atmTrader.config.fiatUSDRate,
        fiat_usdt_amount: self.tx.fiat.toNumber() / self.atmTrader.config.fiatUSDRate,
        snapshot_info_Id : self.atmTrader.tx.snapshotInfoId
    }
    self.atmTrader
        .addAtmOrderBuy(rData).then(res => {
            console.debug(" ==== addAtmOrderBuy ===", res.body)
            if (res.body.code === 0) {
                return self.atmTrader.addAtmOrderBuyRisk(rData).then(()=>{
                    self.atmTrader.updateTx({
                    ...
                    self.saveHyperTx(res.body.data, new Date().toISOString())
                })
            } else {
...
HyperBrain.prototype.atmSellOrderLogic = function () {
    ...
    const validateState = this.handleValidateOutOverTransactionLimit(data.fiat_amount)
    ...
    const decimalN = self.prepareConvertDecimalDigit(coin.decimals)
    const dAmount = (data.fiat_amount / self.atmTrader.tx.coinprice * (1 + (coin.sellFee + coin.sellFeeRate)))
    ...
    this.atmTrader.addAtmOrderSell(
        {
            ...
            price: decimalN.convert(data.fiat_amount /dAmount),
            //price: self.atmTrader.tx.coinprice * (1 + (coin.sellFee + coin.sellFeeRate)),
            price_origin: self.atmTrader.tx.coinprice,
            fiat_usdt_rate: self.atmTrader.config.fiatUSDRate,
            fiat_usdt_amount: data.fiat_amount / self.atmTrader.config.fiatUSDRate,
            snapshot_info_Id: self.atmTrader.tx.snapshotInfoId
        }
    ).then((respond) => {
        // no address
...
HyperBrain.prototype.atmCollectionActionLogic = function () {
    ...
    self.atmTrader.updateTx({ orderNo: address, noPool: true })
    ...
    this.atmTrader.getAtmOrder({
        ...
        if (respond.body.code == 100601) {
            ...
            self.atmTrader.updateTx({ orderDetail: respond.body.data, noPool: true })
            ...
            if (respond.body.data.status == 0) {
                ...
                self._transitionState('atmPaymentSellCollected', {
                    action: "atmPaymentSellCollected",
                    data: {
                        path: `/submitSellCollected`,
                        paymentOrder: respond.body.data,
                        render: self.buildRender(true)
                    }
                })
...
HyperBrain.prototype.initATMTraderEvents = function () {
    ...
    self.atmTrader.on('atmSellOrderUpdate', function () {
        ...
        console.debug(">>>>> before transition", JSON.stringify(order))
        if (order.status == 0) {
        ...
        } else if (order.status == 3) { // other state page
            self.atmTrader.stopPollOrder()
        } else if (order.status == 10) {  // other state page
            self.atmTrader.stopPollOrder()
        }
...
HyperBrain.prototype.atmBuy = async function () {

    // check  number close to last 5 stop
    if (await this.validateInMerchant() != true) {
        console.debug("atmbuy merchant service offline already redirect")
        return
    }
...
HyperBrain.prototype.saveHyperOrderTx = function (order, dispenserInfo, submittedOn = "") {
    const self = this;
    ...
    let newLayout = {
        ...
        chain_name: chain,
        digital_amount: decimalN.convert(order.digital_amount),
...
HyperBrain.prototype.validateOutOverTransactionLimit = function (amount = 0) {
    let overTransactionLimit = false
    let overKYCTransactionLimit = false
    let overNonKYCTransactionLimit = false

    const _amount = BN(amount)

    if (this.atmTrader.tx.needKYC === false) {
        const remain = this.atmTrader.profile.personnel.remaining_sell_amount
        // if choose non kyc, if non kyc over  error
        if (remain < 0) {
            overTransactionLimit = true
            overNonKYCTransactionLimit = true
        } else if (_amount.gt(remain)) {
            overTransactionLimit = true
            overNonKYCTransactionLimit = true
        }
    } else if (this.atmTrader.tx.needKYC === true) {
        // if choose kyc, if kyc over error
        const remain = this.atmTrader.profile.personnel.remaining_kyc_sell_amount
        if (remain < 0) {
            overTransactionLimit = true
            overKYCTransactionLimit = true
        } else if (_amount.gt(remain)) {
            overTransactionLimit = true
            overKYCTransactionLimit = true
        }
    }
    console.debug(">>> debug",overTransactionLimit, overKYCTransactionLimit, overNonKYCTransactionLimit)
    // if amount!=0 half way deposit warning page
    return { overTransactionLimit, overKYCTransactionLimit, overNonKYCTransactionLimit }
}

HyperBrain.prototype.validateInOverTransactionLimit = function (newIn = 1) {
    let overTransactionLimit = false
    let overKYCTransactionLimit = false
    let overNonKYCTransactionLimit = false
    ...
    const amount = this.tx.fiat.add(BN(newIn))
    // console.debug("this.atmTrader.profile.personnel", this.atmTrader.profile)
    if (this.atmTrader.tx.needKYC === false) {
        const remain = this.atmTrader.profile.personnel.remaining_buy_amount
        // if choose non kyc, if non kyc over  error
        if (remain < 0) {
            overTransactionLimit = true
            overNonKYCTransactionLimit = true
        } else if (amount.gt(remain)) {
            overTransactionLimit = true
            overNonKYCTransactionLimit = true
        }
    } else if (this.atmTrader.tx.needKYC === true) {
        // if choose kyc, if kyc over error
        const remain = this.atmTrader.profile.personnel.remaining_kyc_buy_amount
        if (remain < 0) {
            overTransactionLimit = true
            overKYCTransactionLimit = true
        } else if (amount.gt(remain)) {
    ...
    // if amount!=0 half way deposit warning page
    return { overTransactionLimit, overNonKYCTransactionLimit, overKYCTransactionLimit }
}

HyperBrain.prototype.handleValidateOutOverTransactionLimit = function(amount = 0){
    const { overNonKYCTransactionLimit, overKYCTransactionLimit } = this.validateOutOverTransactionLimit(amount)
    if (overKYCTransactionLimit) {
        // warning
        // atmOverWithdrawNonKYCLimit
        this.atmError(hyperError.errMsg["atmOverWithdrawKYCLimit"], {})
        // revert state
        cashOutMachine.dispatch("BACK")
        return false
    } else if (overNonKYCTransactionLimit) {
        // warning
        // atmOverWithdrawNonKYCLimit
        this.atmError(hyperError.errMsg["atmOverWithdrawNonKYCLimit"], {})
        // revert state
        cashOutMachine.dispatch("BACK")
        return false
    }
...
HyperBrain.prototype.handleValidateInOverTransactionLimit = function(){

    const { overNonKYCTransactionLimit, overKYCTransactionLimit } = this.validateInOverTransactionLimit()
    console.debug("overNonKYCTransactionLimit, overKYCTransactionLimit>>>>", overNonKYCTransactionLimit, overKYCTransactionLimit)
    if (overKYCTransactionLimit){
        // warning
        this.atmError(hyperError.errMsg["atmOverDepositKYCLimit"], {})
        //atmOverDepositNonKYCLimit
        // revert state
        cashInMachine.dispatch("BACK")
        return false
    } else if (overNonKYCTransactionLimit) {
        // warning
        this.atmError(hyperError.errMsg["atmOverDepositNonKYCLimit"], {})
        //atmOverDepositNonKYCLimit
        // revert state
        cashInMachine.dispatch("BACK")
        return false
...
HyperBrain.prototype.validateInMerchant = async function () {
    let merchantValid = false
    try{
        const res = await this.atmTrader.isATMOperating()
        merchantValid = res.body.data
    }catch(e){}

    // perform merchant quota validate
    if (merchantValid != true) {
        // so here is the merchant is unable to perform the operation
        // this only check up until notes is inserted
        this.atmTransitionMerchantOffline()
        return false
    }
...
HyperBrain.prototype.saveHyperTx = function (order, submittedOn = "") {
    const self = this;
    let chain = this.atmTrader.tx.chain
    const coin = this.atmTrader.config.coinIndex[this.atmTrader.tx.coinname]
    ...
    let newLayout = {
        id: this.tx.id,
        order_no: this.atmTrader.tx.orderNo || "",
        device_id: this.atmTrader.globalOptions.connectionInfo["ATM-Number"],
    ...
    newLayout.digital_amount = decimalN.convert((this.tx.fiat.toNumber() / coin.price * (1 - (coin.buyFee + coin.buyFeeRate))))
...
HyperBrain.prototype._billRead = async function _billRead(data) {
    ...
    const { overTransactionLimit, overKYCTransactionLimit, overNonKYCTransactionLimit } = this.validateInOverTransactionLimit(data.denomination)
    console.debug("==== overTransactionLimit", overTransactionLimit, "overKYCTransactionLimit", overKYCTransactionLimit, "overNonKYCTransactionLimit", overNonKYCTransactionLimit)
    if (overTransactionLimit){
    ...
    const _amount = this.tx.fiat.add(BN(data.denomination))

   if (this.atmTrader.tx.needKYC === false) {
        if (_amount.lt(this.atmTrader.config.minNonKYCAmount)){
            this.atmError(hyperError.errMsg["atmLessThanDepositKYCLimit"], {})
            this.billValidator.reject()
            return 
        }
    } else if (this.atmTrader.tx.needKYC ===true){
        if (_amount.lt(this.atmTrader.config.minKYCAmount)){
            this.atmError(hyperError.errMsg["atmLessThanDepositKYCLimit"], {})
            this.billValidator.reject()
            return 
        }
    ...
    let allowCloudAllowInsertBill = false
    try{
        let allowInBillRes = await this.atmTrader.addATMInBill({
            "atm_box_id": self.atmTrader.getInBox().id,
            "fiat_amount": data.denomination,
            "fiat_currency": self.atmTrader.config.fiatCurrency,
            "number": 1,
            "order_no": this.atmTrader.tx.orderNo
        });
        console.debug("===== allowInBillRes", allowInBillRes.body)
        allowCloudAllowInsertBill = lodash.get(allowInBillRes, "body.data", false)
    }catch(e){

    }
    if (allowCloudAllowInsertBill == false){
        this.atmError(hyperError.errMsg["atmCashBoxError"], {})
        this.billValidator.reject()
        return 
    }
...
HyperBrain.prototype._initTraderEvents = function _initTraderEvents() {
    ...
    this.atmTrader.on('maintenance', function () { self._toMaintenance() })
    this.atmTrader.on('doneMaintenance', function () { self._doneMaintenance() })
}
