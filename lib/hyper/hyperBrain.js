...
HyperBrain.prototype.atmStartScanQR = function atmStartScanQR() {
    ...
    this.scanner.atmStartScanQR((err, address) => {
        ...
        if (address) {
            ...
            console.log("selfCashOutMachine", selfCashOutMachine.getState())
...
HyperBrain.prototype.atmWithdrawaActionLogic = function () {
    const self = this;
    this.atmInitialWithdraw({ "direction": "cashOut", "cryptoCode": "BTC", fiat: this.atmTrader.tx.orderDetail.fiat || 2 })
    ...
    setTimeout(() => {
        console.log("atmWithdrawaActionLogic emit billDispenserCollected")
        emit({ action: 'billDispenserCollected' })
        console.log("atmWithdrawaActionLogic _completed" )
        self._completed()
        
        setTimeout(() => {
            emit({ action: 'ledsOff' })
            console.log("atmWithdrawaActionLogic emit ledsOff")
            selfCashOutMachine.dispatch("END")
            console.log("atmWithdrawaActionLogic selfCashOutMachine END")
            self.atmConfirmEnd()
        }, 2000)
...
HyperBrain.prototype.atmCollectionActionLogic = function () {
    ...
    this.atmTrader.getAtmOrder({
        ...
        console.debug("atmCollectionActionLogic err")
        if (err.response){
            self.atmError(err.response.body, err)
        }
        const id = Math.random().toString()
