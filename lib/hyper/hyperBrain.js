...
HyperBrain.prototype.atmConfirmDepositLogic = function () {
    ...
    const rData = {
        ...
        fiat_2_u_rate: self.atmTrader.config.fiatUSDRate,
        usd_fiat_amount: self.tx.fiat.toNumber() / self.atmTrader.config.fiatUSDRate
    }
    self.atmTrader
    ...
    this.atmTrader.addAtmOrderSell(
        {
            ...
            price_origin: self.atmTrader.tx.coinprice,
            fiat_2_u_rate: self.atmTrader.config.fiatUSDRate,
            usd_fiat_amount: data.fiat_amount / self.atmTrader.config.fiatUSDRate
        }
    ).then((respond) => {
        // no address
        if (respond.body.code == 0) {
