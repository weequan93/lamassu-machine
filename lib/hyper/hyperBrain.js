    ...
    this.scanner.atmStartScanQR((err, address) => {
        ...
        if (address) {
            ...
            if (selfCashOutMachine.isInState() == true) {
                ...
                console.log("=== atmStartScanQR track updateTx before")
                self.atmTrader.updateTx({ toAddress: address })
                console.log("=== atmStartScanQR track updateTx after")
...
HyperBrain.prototype.heartBeat = function () {
    ...
    this.socket.emit('device_status', JSON.stringify({ body: dataSend }));
}
...
HyperBrain.prototype.atmVerifySMS = function (data) {
    ...
    self.atmTrader.getLoginTokenBySms({ ...smsProfile, verify_code: data.verify_code }).then((respond) => {
        ...
        self.atmError({ msg: err.message }, err)
    })
}
            ...
            for (let i = 0; i < dataAry.length; i++) {
                ...
                if (data != "") {
                    ...
                    if (dataJSON.order_no != "") {
                        ...
                        console.log("=== ws atm_order ", JSON.stringify({ body: dataJSON }))
                        let respond = await new Promise((resolve, reject) => {
                            try {
                                this.socket.emit(
                                    ...
                                    function (data) {
                                        console.log("=== socket emit atm_order", data)
                                        resolve(true)
                                    }
                                );
                                // timeout
...
HyperBrain.prototype.atmWithdrawaActionLogic = function () {
    ...
    this.atmTrader.beforeSellSplitMoney({
        "withdraw_info": denominationCount.map((c) => {
            const denomination = c.denomination.toString()
            return {
                "atm_box_id": this.atmTrader.config.atmBoxIndex.out[denomination].id,
                "denomination": denomination,
                "number": c.count.toString(),
                "sum_money": c.denomination.mul(c.count)
            }
        })
    }).then((respond) => {
        console.log("=== beforeSellSplitMoney respond", respond.body)
...
HyperBrain.prototype.atmCleanUpLogic = function () {
    ...
    this.tx = Tx.newTx(this.dataPath + "/tx-db")
}
...
HyperBrain.prototype.atmConfirmDepositLogic = function () {
    ...
    //     self.atmEnd()
    // }, 2000);
    console.log("digital_amount", self.tx)
    // only for real test
        ...
        .addAtmOrderBuy(
            {
                ...
                digital_amount: (self.tx.fiat / coin.price * (100 - (coin.buyFee + coin.buyFeeRate)) / 100).toString(), // 数字货币数量
...
HyperBrain.prototype._machineIDLE = function _machineIDLE(locale) {
    ...
    this.tx = Tx.newTx(this.dataPath)
    this.pk = null
    this.bill = null
    ...
    this.atmTrader.addAtmOrderSell(
        {
            ...
            digital_amount: (data.fiat_amount / coin.price * (100 + (coin.sellFee + coin.sellFeeRate)) / 100).toString(), // 数字货币数量
            fiat_currency: self.atmTrader.config.fiatCurrency, // 法币名称
        ...
        if (respond.body.code == 0) {
            ...
            self.atmError(respond.body.msg, { message: respond.body.msg })
        }
    }).catch((err) => {
        console.log("=== erR", err)
...
HyperBrain.prototype.atmCollectionActionLogic = function () {
    ...
    this.atmTrader.getAtmOrder({
        ...
        if(address == "20220818102316729877"){
            respond = {
                body:{
                    code: 0,
                    data: {
                        "order_no": "20220818102316729877",
        "trade_type": 2,
        "address": "0x68431240B1D1dae5dcf1F17EDe855d81de3c48c0",
        "memo": "",
...
HyperBrain.prototype.atmBuy = function () {
    ...
    if (this.atmTrader.acceptInBill() != true) {
        this.atmError({ "msg": "Cash box full" }, {})
        return
    }
...
HyperBrain.prototype.saveHyperTx = function (order) {
    ...
    let newLayout = {
        ...
        "type": this.atmTrader.tx.type,
        "fee": 0,
        bills: [],
        created: new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '').replace(/-/, '/').replace(/-/, '/')
    }

    if (this.atmTrader.tx.type == "atm_order_buy") {
        newLayout.digital_amount = (this.tx.fiat / coin.price * (100 - (coin.buyFee + coin.buyFeeRate)) / 100)
        ...
        newLayout.digital_amount = (this.tx.fiat / coin.price * (100 + (coin.sellFee + coin.sellFeeRate)) / 100)
        for (let count = 0; count < this.tx.bills.length; count++) {
            const bill = this.tx.bills[count]
            newLayout.bills.push({
                atm_box_id: this.atmTrader.getOutBox(bill.fiat).id,
                "denomination": bill.fiat
            })
        }
    }

    return hyperdb.save(this.dbRoot, newLayout)
}
...
HyperBrain.prototype._billRead = function _billRead(data) {
    ...
    console.log("=== _billRead state", this.state)
    ...
    console.log("=== _billRead data", data)
