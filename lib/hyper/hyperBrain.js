...
HyperBrain.prototype.heartBeat = async function (status =0 ) {
    ...
    const dataSend = {
        latency: parseFloat(networkHeartBeat[0].averageResponseTime),
        network_speed: parseFloat(downloadSpeed[0].speed),
        packet_loss: parseFloat(networkHeartBeat[0].packetLoss),
...
HyperBrain.prototype.saveHyperTx = function (order) {
    ...
    newLayout.digital_amount = (this.tx.fiat.toNumber() / coin.price * (100 - (coin.buyFee + coin.buyFeeRate)) / 100)
