...
HyperBrain.prototype.atmVerifySMS = function (data) {
    ...
    self.atmTrader.getLoginTokenBySms({ ...smsProfile, verify_code: data.verify_code }).then((respond) => {
        ...
        console.error("err _atmSendSMS", err)
    })
}
...
HyperBrain.prototype.atmProcessBack = function () {
    ...
    if (cashInMachine.isInState() == true) {
        ...
        return
    } else if (this.state == 'atmSuccessLogin' || this.state == 'atmbuy') {
        this.atmTrader.reset()
        this.atmTransitionLogin()
        return
    }
    console.debug("_atmProcessBack", this.state)
}
...
HyperBrain.prototype.atmConfirmAddress = function () {
...
HyperBrain.prototype.atmGetWalletCoinAddressLogic = function () {
    ...
    return self.atmTrader.getCoinAddress({
        ...
        self.updateTx({ toAddress: respond.body.data.address })
...
HyperBrain.prototype.atmWithdrawaActionLogic = function () {
    const self = this;
    console.debug("atmWithdrawaActionLogic 1",)
    this.atmInitialWithdraw({ "direction": "cashOut", "cryptoCode": "BTC", fiat: this.atmTrader.tx.orderDetail.fiat || 1 })

    this.tx.direction = 'cashOut'
    this._chooseFiat("cashOut")

    console.debug("atmWithdrawaActionLogic 1", this.tx.direction) 
    this.toDeposit()
...
HyperBrain.prototype.atmConfirmDepositDetailLogic = function () {
    this.atmTransitionConfirmDepositDetail()
}

HyperBrain.prototype.atmConfirmDepositInformationLogic = function () {
    this.atmTransitionConfirmDepositInformation()
}

HyperBrain.prototype.atmCashInCleanUpLogic = function () {
    console.log("atmCashInCleanUpLogic")
    this.atmTrader.resetTx()
    this.tx.id = uuid.v4()
    this.tx.fiat = BN(0)
    this.tx.cryptoAtoms = uuid.v4()
    this.tx.bills = []

    this.tx.dirty = true
    this.tx.termsAccepted = false
    this.tx.txVersion = 0
}
...
HyperBrain.prototype.atmConfirmDepositLogic = function () {
    ...
    self.atmTrader.getCoinPrice({
        ...
        return self.atmTrader
            .addAtmOrderBuy(
                {
                    digital_currency: self.atmTrader.tx.coinname, //coinName, // 数字货币名称
                    chain_name: "", //chainName, // 链名称
                    // digital_amount: (putAmount / coinsRate[coinName]).toString(), // 数字货币数量
                    digital_amount: (self.tx.fiat / data.rate).toString(), // 数字货币数量
                    fiat_currency: "usd", // 法币名称
                ...
                if (res.code === 200) {
                    self.atmTrader.updateTx(res.body.data)
                    self.atmSubmitBill()
                    setTimeout(() => {
...
HyperBrain.prototype.atmSellOrderLogic = function () {
    ...
    self.atmTrader.getCoinPrice({
        ...
        return this.atmTrader.addAtmOrderSell(
            {
                digital_currency: "btc", // 数字货币名称
                chain_name: "", // 链名称
                //digital_amount: (data.flat_amount / 1/*rate*/).toString(), // 数字货币数量
                digital_amount: (data.flat_amount / datarate.rate).toString(),
                fiat_currency: "usd", // 法币名称
                fiat_amount: data.flat_amount//"100" // 法币数量
            }
        ).then((respond) => {
...
HyperBrain.prototype.atmCollectionActionLogic = function () {
    const address = cashOutMachine.getData('address')
    const self = this;
    ...
    this.atmTrader.getAtmOrder({
        ...
        if (respond.body.data.status == 0) {
            this.atmTransitionWithdrawProcessStatePending({
                paymentOrder: respond.body.data
            })
        } else if (respond.body.data.status == 1) {
            this.atmTransitionWithdrawProcessStateCompleted({
                paymentOrder: respond.body.data
            })
        } else if (respond.body.data.status == 2) {
            this.atmTransitionWithdrawProcessStateWithdraw({
                paymentOrder: respond.body.data
            })
        } else if (respond.body.data.status == 3) {
            this.atmTransitionWithdrawProcessStateWithdrawing({
                paymentOrder: respond.body.data
            })
        } else if (respond.body.data.status == 10) {
            this.atmTransitionWithdrawProcessStateRefunded({
                paymentOrder: respond.body.data
            })
        }
    }).catch((err) => {
        self.atmError(hyperError.errMsg["atmRetriveOrderErr"], err)
...
HyperBrain.prototype.atmSuccessLogin = function () {
    ...
    } else if (this.tx.direction == 'cashOut' && this.real != false) {
...
HyperBrain.prototype.atmSuccessLoginLogic = function () {
    const rec = {}
    // find redirect
    if (this.tx.direction == 'cashOut' && this.real != false ) {
        rec.action = "atmSuccessLogin"
        rec.data = { path: '/scanCollectionAddress' }
        this.atmStartScanQR()
    }
...
HyperBrain.prototype.initATMTraderEvents = function () {
    ...
    self.atmTrader.on('atmSellOrderUpdate', function () {
        self.browser().send({ action: 'atmSellOrderUpdate', data: self.atmTrader.tx.orderDetail })
    })
