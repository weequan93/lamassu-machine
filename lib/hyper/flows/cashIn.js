...
const cashInMachine = xstate.Machine({
    ...
    states: {
        ...
        atmCoinInLanding: {                  // coinin 首页
            onEntry: ["atmInitialBill", "atmTransitionCoinInLanding", "validateInMerchant"],
            on: {
                SELECTCOIN: { "atmPaymentAddress": { actions: []}},
                BACK: { "atmWaitingLogin": { actions: ["atmMachineProcessBack"] } },
                END: { "idle": { actions: ['atmAllInitialState'] } },
            }
        },
        atmPaymentAddress: {                // 等待提供地址
            onEntry: ["atmTransitionPaymentAddress", "atmStartScanQR","validateInMerchant", "handleValidateInOverTransactionLimit"],
        ...
        atmGetWalletAddress: {              // 获取wallet地址
            onEntry: ["atmGetWalletCoinAddressLogic", "validateInMerchant"],
            on: {
        ...
        atmConfirmAddress: {                // 确认地址
            onEntry: ["atmTransitionConfirmAddress", "stopEmptyBillValidator", "validateInMerchant"],
            on: {
        ...
        atmInitialBillIn: {                  // 开始收币
            onEntry: ["atmTransitionInitialBillIn", "atmAcceptingFirstBill", "validateInMerchant"],
            on: {
        ...
        confirmDeposit: {                     // 确认 执行买
            ...
            on: {
                FAIL: { 'atmConfirmDepositInformation': { actions: [] } },
                END: { "idle": { actions: ['atmAllInitialState'] } },
            }
        },
