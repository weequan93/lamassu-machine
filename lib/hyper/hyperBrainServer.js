...
const HyperBrainExtend = function (HyperBrain) {
    ...
    HyperBrain.prototype.hyperATMSocket = async function (brain){
        console.log(brain.connectionInfoPath)
        console.debug("=== brain config", JSON.stringify(brain.config))
        ...
        brain.socket.on('open', () => {
            console.log("=== hyper socket connected using token = ", res.body.data.token, socket.readyState, socket.OPEN)
        ...
        brain.socket.on('message', (req) => {
            ...
            try{
                socketProcessEvent(JSON.parse(req));
            }catch(e){
