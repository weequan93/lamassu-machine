...
const HyperBrainExtend = function (HyperBrain) {
        ...
        const server = http.createServer(function (req, res) {
            ...
            if ('OPTIONS' == method) {
                ...
                brain.atmTrader.getLogoAuthCode().then((respond) => {
                    // TODO register callback redirection
                    brain.atmTrader.poolLogin(respond.body.data)
    ...
    HyperBrain.prototype.hyperATMSocket = async function (brain) {
        ...
        socket.on('open', () => {
            ...
            // close current socket & switch over
            if (brain.socket && brain.socket.readyState == socket.OPEN) {
                //brain.socket.close()
                brain.socket = socket;
            }
            brain.socket = socket;
            ...
            // overwrite it
            logging.initWSWriteLog(brain)
        });
        socket.on('disconnected', () => {
            console.log("=== hyper socket disconnected"/*, res.body.data.token*/)
        });

        socket.on('error', () => {
            this.hyperATMSocket(brain)
            console.log("=== hyper socket error"/*, res.body.data.token*/)
        });

        socket.on('connect_error', (connect_error) => {
            console.log("=== hyper socket connect_error"/*, res.body.data.token, connect_error*/)
        });

        socket.on('connect_timeout', () => {
            console.log("=== hyper socket connect_timeout"/*, res.body.data.token*/)
        });
        socket.on('close', () => {
            this.hyperATMSocket(brain)
            console.log("=== hyper socket close"/*, res.body.data.token*/)
        });
    ...
    HyperBrain.prototype.listenSocketEvent = function () {
        this.socket.on("machine_control", (action, callback) => {
            if (action != "" && action != null) {
                // wait event
                if (action == "unpair") {
                    // nothing to do no such feature yet
                    this.atmTrader.emit("unpair")
                } else if (action == "reboot") {
                    this.atmTrader.emit("reboot")
                } else if (action == "shutdown") {
                    this.atmTrader.emit("shutdown")
                } else if (action == "restart") {
                    // modify restart our service
                    this.atmTrader.emit("restartServices")
                } else if (action == "software_update") {
                    // watch dog continue always on. 
                    // start hyper-updater
                    //startUpdater()
                }
