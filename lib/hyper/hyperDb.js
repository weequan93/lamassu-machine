...
module.exports = { save, prune, clean, newDB, getTxLogFiles, getLogFile, clear }
...
function save (dbRoot, tx) {
  ...
  console.log("dbRoot", dbRoot, dbName, JSON.stringify(tx))
  const date = new Date().toISOString()
  return fs.appendFile(path.resolve(dbRoot, dbName), JSON.stringify({...tx, modifyOn: date }) + '\n')
}
...
function clear (dir,file) {
  console.log("clear ",'mv ' + dir+"/"+file+ " "+ dir+"/ar."+file)
  cp.exec('mv ' + dir+"/"+file + " "+ dir+"/ar."+file, err => {
...
function clean (dbRoot) {
  ...
  if (!fs.existsSync(dbRoot)) return
  const files = fs.readdirSync(dbRoot)
  if (files.length) {
...
function newDB(dbRoot, uuid){
  dbName = 'hypertx-db-' + uuid + '.dat'
  return fs.mkdir(dbRoot)
    .catch(() => {})
    .then(() => fs.writeFile(path.resolve(dbRoot, dbName), ''))
}

function getTxLogFiles (dbRoot) {
  return fs.readdir(dbRoot).then(files => {
    return files
  })
}

// order_no not ""
function getLogFile (dbRoot, logsPath) {
  return fs.readFile(dbRoot+"/"+logsPath, {encoding: "utf-8"}).then(data => {
    return data;
  })
}