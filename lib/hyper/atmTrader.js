...
const _riskRequest = require('./riskRequest')
...
const ATMTrader = function (protocol, clientCert, connectionInfo, dataPath, relayedModel) {
    ...
    this.riskRequest = options => _riskRequest.request(this.configVersion, {
        ...globalOptions
        , token: this.profile.token
    }, options)
...
ATMTrader.prototype.generateConfig = function (atmConfigs){
    ...
    const base = Object.assign(self.config || {}, {
        ...
        _minKYCAmount: 0,
        _maxKYCAmount: 0,
        _predefinedNonKYCWithdrawList: [],
        _predefinedKYCWithdrawList: []
        // atmBoxIn: []
    }) 
    if (atmConfigs.fiat_currency && Array.isArray(atmConfigs.fiat_currency) && atmConfigs.fiat_currency.length > 0) {
        base.fiatCurrency = atmConfigs.fiat_currency[0]
        base.fiatSymbol = bills[atmConfigs.fiat_currency[0]].symbol
    }
    ...
    if (atmConfigs.max_trade_amount != null && atmConfigs.max_trade_amount >= 0) {
        base._maxTradeAmount = atmConfigs.max_trade_amount
    }
    if (atmConfigs.max_trade_kyc_amount != null && atmConfigs.max_trade_kyc_amount >= 0) {
        base._maxTradeKYCAmount = atmConfigs.max_trade_kyc_amount
    }
    if (atmConfigs.min_amount != null){
        base._minNonKYCAmount = atmConfigs.min_amount
    }
    if (atmConfigs.max_amount != null) {
        base._maxNonKYCAmount = atmConfigs.max_amount
    }
    if (atmConfigs.min_amount_kyc != null) {
        base._minKYCAmount = atmConfigs.min_amount_kyc
    }
    if (atmConfigs.max_amount_kyc != null) {
        base._maxKYCAmount = atmConfigs.max_amount_kyc
    }
    if (atmConfigs.amount_list && Array.isArray(atmConfigs.amount_list)) {
        base._predefinedNonKYCWithdrawList = atmConfigs.amount_list
    }
    if (atmConfigs.amount_list_kyc && Array.isArray(atmConfigs.amount_list_kyc)) {
        base._predefinedKYCWithdrawList = atmConfigs.amount_list_kyc
    }
    return base;
}
...
ATMTrader.prototype.updateTodayAmount = function updateTodayAmount() {
    ...
    return this.getTradeAmount().then((respond)=>{
        ...
        //     }
        // }
        let remaining_buy_amount = this.config.maxTradeAmount - respond.body.data.buy_amount
        let remaining_sell_amount = this.config.maxTradeAmount - respond.body.data.sell_amount

        let remaining_kyc_buy_amount = this.config.maxTradeKYCAmount - respond.body.data.buy_amount
        let remaining_kyc_sell_amount = this.config.maxTradeKYCAmount - respond.body.data.sell_amount
...
ATMTrader.prototype.pollHandler = function (res) {
    ...
    new Promise((resolve) => {
        ...
        if (cname == ""){
            if (this.config.fiatCurrency == 'usd' || this.config.fiatCurrency == 'USD') {
                this.config.fiatUSDRate = 1;
            } else {
                // get fiat rate
                try{
                    const res = await self.getFiatUsdRate({ fiat_currency: this.config.fiatCurrency })
                    if (res.body.code != 0) {
                        this.config.fiatUSDRate = 0
                    }
                    this.config.fiatUSDRate = lodash.get(res, "body.code.rate")
                    if (typeof this.config.fiatUSDRate == "string"){
                        this.config.fiatUSDRate = parseFloat(this.config.fiatUSDRate)
                    }
                }catch(e){
                    console.error("getFiatUsdRate error ", e.message)
                    this.config.fiatUSDRate = 0
                }
            }
            ...
            for (let coini in _c) {
                ...
                try {
                   ...
                   // console.debug(">>>>>>>>>>>>>>>>>>>>>>> getCoinPrice", coin._chain, coin.symbol,res.body)
                    ...
                    coin.price = res.body.data.rate * self.config.fiatUSDRate
                   // console.debug(">>>>>>> ,", coin.price, res.body.data.rate , self.config.fiatUSDRate )
                    ...
                    if(coin.name=="hpy_test"){
                        coin.price = 1 * self.config.fiatUSDRate;
                        coin.sellFee =  0.01
                        coin.buyFee = 0.01
            ...
            // replace kyc
            self.config.maxTradeAmount = self.config._maxTradeAmount
            self.config.maxTradeKYCAmount = self.config._maxTradeKYCAmount
            self.config.minNonKYCAmount = self.config._minNonKYCAmount
            self.config.maxNonKYCAmount = self.config._maxNonKYCAmount
            self.config.minKYCAmount = self.config._minKYCAmount
            self.config.maxKYCAmount = self.config._maxKYCAmount
            self.config.predefinedNonKYCWithdrawList = self.config._predefinedNonKYCWithdrawList
            self.config.predefinedKYCWithdrawList = self.config._predefinedKYCWithdrawList
        }
        this.emit("atmSync")
    })
...
// risk 
ATMTrader.prototype.addAtmOrderBuyRisk = function addAtmOrderBuyRisk(data) {
    console.log("=== addAtmOrderBuyRisk", JSON.stringify(data))
    return this.riskRequest({
        path: '/risk/atm/add-atm-order-buy',
        method: 'POST',
        body: data
    })
}
...
// 查询法币兑u汇率
ATMTrader.prototype.getFiatUsdRate = function addATMInBill(data) {
    console.log("=== getFiatUsdRate", JSON.stringify(data))
    return this.request({
        path: '/client/getFiatUsdRate',
        method: 'POST',
        body: data
    })
}
...
// 获取交易快照
ATMTrader.prototype.getSnapshotInfo = function getSnapshotInfo(data) {
    console.log("=== getSnapshotInfo", JSON.stringify(data))
    return this.request({
        path: '/client/getSnapshotInfo',
        method: 'POST',
        body: data
    })
}
