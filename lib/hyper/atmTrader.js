...
ATMTrader.prototype.updateTodayAmount = function updateTodayAmount() {
    ...
    return this.getTradeAmount().then((respond)=>{
        console.debug("respond", respond.body)
...
ATMTrader.prototype.pollHandler = function (res) {
    ...
    new Promise((resolve) => {
        ...
        if (cname == ""){
            for (let coin of self.config._coins) {
                try {
                    const res = await self.getCoinPrice({ chain: coin._chain, digital_currency: coin.name, fiat_currency: self.config.fiatCurrency || "usd" })
                    if(res.body.code != 0){
                        //console.debug("res coin", res.body, { chain: coin._chain, digital_currency: coin.name, fiat_currency: self.config.fiatCurrency || "usd" })
                    }
            ...
            self.config.coinIndex = lodash.keyBy(self.config._coins, 'name');  
