...
ATMTrader.prototype.generateConfig = (atmConfigs) => {
    const base = {
        coins: [],
        coinIndex: {},
        fiatCurrency: 'usd',
    ...
    if (atmConfigs.crypto_coin && Array.isArray(atmConfigs.crypto_coin)) {
        // group by coin
        base.coinIndex = atmConfigs.crypto_coin.reduce((acc, curr) => {
            // coin key
            if (!acc[curr.symbol]) {
                acc[curr.symbol] = {
                    "symbol": curr.symbol,
        ...
        base.coins = Object.values(base.coinIndex)
    }
...
ATMTrader.prototype.syncLogs = function syncLogs() {
    console.log("=== syncLogs remove old 5 day log")
...
ATMTrader.prototype.pollOrder = function pollOrder() {
    ...
    this.orderPolling = setInterval(() => {
        ...
        self.getAtmOrder({
            ...
            console.log("=== pollOrder ", self.tx, respond.body)
...
ATMTrader.prototype.poll = function poll() {
    console.log("*** send state to websocket")
    console.log("*** set network down if pooling got problem")
...
ATMTrader.prototype.pollHandler = function (res) {
    ...
    new Promise((resolve) => {
        ...
        for (let coin of self.config.coins) {
            try {
                const res = await self.getCoinPrice({ digital_currency: coin.name, fiat_currency: self.config.fiatCurrency || "usd" })
                ...
                //console.log("Err getCoinPrice = ", JSON.stringify(coin), err.message, JSON.stringify(res))
...
ATMTrader.prototype.addAtmOrderSell = function addAtmOrderSell(data) {
    console.log("=== addAtmOrderSell", data)
    return this.request({
...
ATMTrader.prototype.getAtmOrder = function getAtmOrder(data) {
    return this.request({
        path: `/client/get-atm-order?orderNo=${data.order_no}`,
        method: 'GET'
    })
}
...
ATMTrader.prototype.beforeSellSplitMoney = function beforeSellSplitMoney(data) {
    console.log("=== beforeSellSplitMoney", data)
    return this.request({
