...
Updater.prototype._preUpdate = function _preUpdate(path) {
  ...
  return got(gotOptions.path, gotOptions).then((r) => {
    if(r.body.data.status == 1){
      ...
      self.downloadurl = "https://atm-backend.s3.ap-south-1.amazonaws.com/release/latest.zip" //r.body.data.url
    }
    return r
  }).catch((err)=>{
    console.log("err", err)
...
Updater.prototype._update = function _update() {
  this._preUpdate("/version/info")
  if (this._readyForDownload()) this._download(this.downloadurl)
}
...
Updater.prototype._downloadFile = function _downloadFile(res) {
  ...
  var contentVersion = res.headers['x-amz-meta-version']
