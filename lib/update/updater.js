...
Updater.prototype._readyForDownload = function _readyForDownload () {
  ...
  console.log("_readyForDownload", this.version, this.newerVersion)
...
Updater.prototype._preUpdate = function _preUpdate (path) {
  ...
  var req = https.request(options, function (res) {
    var serverVersion = res.headers['x-amz-meta-version']
    console.log("_preUpdate", serverVersion, self.version )
   // if(serverVersion != this.version){
      self.newerVersion = serverVersion;
      // ok, found version different
      // check for is update next version / pull latest
   // }
...
Updater.prototype._update = function _update () {
  this._preUpdate("/release/latest.zip")
  if (this._readyForDownload()) this._download("/release/latest.zip")
}
...
Updater.prototype._download = function _download (path) {
  ...
  if (this.downloading) return
  console.log("_download downloading", this.downloading)
  var self = this
...
console.log("softwareConfig", softwareConfig)
...
console.log("config outside", config)
