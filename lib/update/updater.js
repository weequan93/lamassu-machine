...
function fetchVersion(config) {
  const str = fs.readFileSync(config.updater.packageJsonDir + '/package.json')
...
Updater.prototype._init = function _init() {
  ...
  var downloadDir = this.config.updater.downloadDir
  var packagePath = this.config.updater.downloadDir + '/latest.zip'
  // var packagePath = this.config.downloadDir + '/update.zip'
...
Updater.prototype._httpsOptions = function _httpsOptions() {
  ...
  var options = {
    host: config.updater.host,
    port: config.updater.port,
    path: config.updater.path,
    protocol: "https:",
    hostname: config.updater.host,
    // key: this.key,
    // cert: this.cert,
    // ca: this.ca,
...
Updater.prototype._readyForDownload = function _readyForDownload(shouldDownload) {
  ...
  console.log("_readyForDownload this.version=", this.version, "this.newerVersion=", this.newerVersion)
  if (this.version < this.newerVersion) {
...
Updater.prototype._preUpdate = function _preUpdate(path) {
  ...
  const gotOptions = {
    ...
    method: "post",
    path: options.path,
    body: { "build_number": this.version },
    timeout: 20000,
  ...
  return got(gotOptions.path, gotOptions).then((r) => {
    if(r.body.data.status == 1){
      self.newerVersion = r.body.data.build_number
      self.downloadurl = r.body.data.url
      console.log("download url", r.body.data.url, "version", r.body.data.build_number)
      return true
    }
    return false
  }).catch((err)=>{
...
Updater.prototype._update = function _update() {
  this._preUpdate("/version/info").then((shouldDownload)=>{
    if (this._readyForDownload(shouldDownload)) this._download(this.downloadurl)
  })
}
...
Updater.prototype._download = function _download(path) {
  const self = this
  if (this.downloading) return
...
Updater.prototype._downloadFile = function _downloadFile(res) {
  ...
  */
  ...
  res.on('end', function () {
    ...
    if (res.complete) {
      self._extract({
        rootPath: self.config.updater.extractDir,
...
Updater.prototype._extract = function _extract(fileInfo) {
  var self = this
  console.log("_extract fileInfo", fileInfo)
...
Updater.prototype._triggerWatchdog = function _triggerWatchdog() {
  var donePath = this.config.updater.extractDir + '/done.txt'
...
var CONNECTION_INFO_PATH = path.resolve(__dirname, '../../data/connection_info.json')
...
var connectionInfo = JSON.parse(fs.readFileSync(CONNECTION_INFO_PATH))

var config = softwareConfig
config.updater = _.merge(config.updater, deviceConfig)
//config.updater = _.merge(config.updater, connectionInfo)
config.updater = _.merge(config.updater, softwareConfig)
config.updater = _.merge(config.updater, connectionInfo)

//config.updater = { ...config.updater, ...deviceConfig, ...connectionInfo }
// config.updater.certs = config.brain.certs
config.updater.dataPath = dataPath
