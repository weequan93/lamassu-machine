...
var Extractor = function (config) {
  ...
  this.skipVerify = true //config.skipVerify
}
...
Extractor.prototype._verifySig = function _verifySig (cb) {
 ...
 // var pubKey = this.lamassuPubKey
  ...
  input.on('end', function () {
    console.log(sig)
    var success = true//verifier.verify(pubKey, sig, 'hex')
...
Extractor.prototype._extractTree = function _extractTree (cb) {
  const self= this
  console.log("_extractTree", self.filePath, self.rootPath)
  cp.exec("mv "+self.filePath+" " + self.rootPath+"/latest.zip", null, function(){
    console.log("perform unzip", "unzip "+self.rootPath+"/latest.zip")
    cp.exec("unzip "+self.rootPath+"/latest.zip -d "+self.rootPath, null, cb)
  })
...
Extractor.prototype._cleanUp = function _cleanUp (cb) {
  ...
  console.log(command)
  cp.exec(command, null, function(){
    cp.exec("mkdir "+self.rootPath, null, cb)
  })
}
